...
class DashboardKPSI {
  ...
  factory DashboardKPSI.fromJson(Map<String, dynamic> json) {
    return DashboardKPSI(
      ...
      status: json['status'],
      status_ok: json['status_ok'],
    );
  }
}

List<DashboardKPSI> dashboardKpsiFromJson(String jsonData) {
  final data = json.decode(jsonData);
  return List<DashboardKPSI>.from(
      data.map((item) => DashboardKPSI.fromJson(item)));
}

String dashboardKpsiToJson(Dosen data) {
  final jsonData = data.toJson();
  return json.encode(jsonData);
}
...
class LoginIn {
  ...
  factory LoginIn.fromJson(Map<String, dynamic> map) {
    return LoginIn(nimnik: map["nimnik"], password: map["password"]);
  }
